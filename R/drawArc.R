...
#' Draw Elliptic Arc(s) 
#' 
#' Draw one or more elliptic (or circular) arcs from \code{startAngle} to
#' \code{endAngle} on an existing plot using classic graphics. 
#' 
...
#' radius.x. 
#' @param startAngle a scalar or a vector of starting angles in radians. 
#' @param endAngle a scalar or a vector of ending angles in radians. 
...
#' drawArc(x = pi/2, y = 0, rx = 1, startAngle = pi/4, endAngle = 3*pi/4, col="red")
#' 
...
#'
#' @export
drawArc <- function (x = 0, y = x, rx = 1, ry = rx, startAngle = 0,
                     endAngle = 2*pi, nv = 100, col = par("col"), lty = par("lty"),
  ...
  for (i in 1L:attr(lgp, "maxdim")) {
    dtheta <- lgp$endAngle[i] - lgp$startAngle[i]
    
    theta <- seq(from = 0,
                 to = ifelse(dtheta < 0, dtheta + 2 * pi, dtheta),
                 length.out = lgp$nv[i]) + lgp$startAngle[i]
